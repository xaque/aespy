...
assert(aes.subWord(0x00102030) == 0x63cab704)
assert(aes.subWord(0x40506070) == 0x0953d051)
assert(aes.subWord(0x8090a0b0) == 0xcd60e0e7)
assert(aes.subWord(0xc0d0e0f0) == 0xba70e18c)

assert(aes.rotWord(0xff000000) == 0xff)
assert(aes.rotWord(0x0e000000) == 0x0e)
assert(aes.rotWord(0x12345678) == 0x34567812)
assert(aes.rotWord(0x09cf4f3c) == 0xcf4f3c09)
assert(aes.rotWord(0x2a6c7605) == 0x6c76052a)

key128 = np.array([0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6,
				0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c], dtype=np.int8)

expanded128 = [0x2b7e1516, 0x28aed2a6, 0xabf71588, 0x09cf4f3c,
...
assert(aes.expandKey(key128) == expanded128)
...
assert((aes.addRoundKey(mix, aes.getRoundKey(aes.expandKey(key), 1)) == rounds).all())

in1 = np.array([0x32, 0x43, 0xf6, 0xa8, 0x88, 0x5a, 0x30, 0x8d,
				0x31, 0x31, 0x98, 0xa2, 0xe0, 0x37, 0x07, 0x34], dtype=np.int8)

result = np.array([0x39, 0x25, 0x84, 0x1d, 0x02, 0xdc, 0x09, 0xfb,
				   0xdc, 0x11, 0x85, 0x97, 0x19, 0x6a, 0x0b, 0x32], dtype=np.int8)
